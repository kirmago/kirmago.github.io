...
const TambahDetailPostingan = ({ sjudul, setIsDarkMode, isDarkMode, outputElements, setOutputElements, setHiddenUi, hiddenUi}) => {
  ...
  switch (elementType) {
    ...
    case "urlGambar":
      return <img className="single-img" key={value} src={value} alt="Gambar" />;
    case "subjudul":
...
*/
  ...
  return (
    <>
      <div className="overgas" style={{ marginTop: `${hiddenUi ? "5vh" : "0px"}` }}>
  ...
  <div className="preview">
    {elements.map((element, index) => (
      <div key={index} className="preview-wrapper">
        {Object.keys(element).map((key) => (
          key.startsWith('multipleImage') ? (
            <div key={key} className="multiple-wrapper">
              {renderElement({ [key]: element[key] })}
            </div>
          ) : (
            <div key={key}>{renderElement({ [key]: element[key] })}</div>
          )
        ))}
      </div>
    ))}
      ...
      <>
        ...
        <>
        {!inputValue ? (
        <button id="hsx" onClick={() => samakanJudul()}>
        ...
        <>
            ...
            <input className="ols" type="file" id="file-input" onChange={handleAddImage}  name="file-input" />
            <label className="addMage" id="file-input-label" for="file-input"><span className="material-symbols-rounded">upload_file</span></label>
        </>
        )}
      ...
      <div className="element-type-buttons">
        <div className="hideui">
          <button onClick={() => setHiddenUi(e => !e)}>
            <span className="material-symbols-rounded" style={{ transform: `${hiddenUi ? "rotate(180deg)" : "rotate(0deg)"}`, transition: "transform .5 ease-in-out" }}>straight</span>
          </button>
        </div>
        ...
        {tambahSubJudul && (
        <button onClick={() => handleElementTypeChange("subjudul")}>
        ...
        {tambahParagraf && (
        <button onClick={() => handleElementTypeChange("paragraf")}>
          <span className="material-symbols-rounded">done</span>
        </button>
        )}
        
        {!isVisiblePara || !isVisibleGam || !isVisibleSuju ? (
        ...
        <>
        {!tambahSubJudul || !tambahGambar || !tambahParagraf ? (
          <button id="hsx" onClick={() => handleElementTypeChange("judul")}>
            <span className="material-symbols-rounded">done</span>
          </button>
         ) : ""}
        </>
