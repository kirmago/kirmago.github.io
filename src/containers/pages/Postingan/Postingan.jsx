...
import { deleteDoc, doc, updateDoc, getDoc, collection, onSnapshot } from 'firebase/firestore';
...
const Postingan = ({setIsDarkMode, isDarkMode}) => {
  ...
  useEffect(() => {
    const filteredPostingan = postingan.filter(post => {
      const judulMatch = post.judul.toLowerCase().includes(searchTerm.toLowerCase());
      const deskripsiMatch = post.deskripsiSingkat.toLowerCase().includes(searchTerm.toLowerCase());
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      const querySnapshot = await onSnapshot(collection(db, 'postingan'), (snapshot) => {
        const items = [];
  ...
  const handleUnLike = async (postId) => {
    ...
    // Memeriksa apakah pengguna sudah memberi suka pada postingan ini
    const unlikedPosts = JSON.parse(localStorage.getItem('unlikedPosts')) || [];
    ...
    if (postDoc.exists()) {
      ...
      try {
        ...
        localStorage.setItem('unlikedPosts', JSON.stringify(unlikedPosts));
      } catch (error) {
        setUnLikeError('Gagal tidak menyukai postingan. Coba lagi nanti.');
  ...
  return (
    <>
      ...
      <div className="buat-postingan">
        <span onClick={buatPostingan} className="material-symbols-rounded">post_add</span>
      </div>
      )}
      </>
      ) : ""}
      ...
      <div className="search-wrapper">
        <div className="search">
          ...
          <div className="search-right">
            <input type="text" placeHolder="Cari postingan.." value={searchTerm} 
      ...
      {postingan.length > 0 ? (<>
        {postingan.map((postinganNya) => {
          const judulPotong = postinganNya.judul.substring(0, 20)
          const judulTampilan = judulPotong.length === 20 ? `${judulPotong}...` : judulPotong;

           return (<>
           
          <div className="listedx">
          <li className="single-listed-postingan" key={postinganNya.id} onMouseDown={tahanDuaMdetik} >
            
            {user && user.role === "admin" && (
            <div id="deletePost">
              <span onClick={() => deleteDocumentByid(postinganNya.id)} className="material-symbols-rounded">delete</span>
            </div>
            )}
            
            
            <Link to={`/postingan/${postinganNya.id}`}>
            <div className="slp-top">
              <img src={postinganNya.thumb} alt="foto siswa" />
            </div>
            <div className="slp-bottom">
              <div className="title-des">
              <h3>{judulTampilan}</h3> 
              <p>{postinganNya.deskripsiSingkat.substring(0, 30)}...</p>
              <i>{postinganNya.tanggalPosting}</i>
              </div>
            </div>
            </Link>
          </li>
          ...
          <div className="user-who-post">
            <img src={postinganNya.authorPfp ?  postinganNya.authorPfp : "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcRNyfBW5GwUReZfJs8r8lA-VhnO2-CVGfhZ_En7WP-xjjOnxXADTrAEClIF&s=10"} alt="foto profil" />
            <p id="username">{!postinganNya.author ? "unknown" : postinganNya.author}</p>
            <p id="usertype">admin</p>
          </div>
          <div className="likesys">
            <div onClick={() => handleLike(postinganNya.id)} className="likeds">
              <button><span className="material-symbols-rounded">thumb_up</span></button>
              <p>{!postinganNya.likes ? "0" : postinganNya.likes}</p>
            </div>
             <div onClick={() => handleUnLike(postinganNya.id)} className="likeds">
              <button><span className="material-symbols-rounded">thumb_down</span></button>
              <p>{!postinganNya.unlikes ? "0" : postinganNya.unlikes}</p>
            </div>
          </div>
          </div>
          </div>
